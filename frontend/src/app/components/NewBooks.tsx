                        ...
                        <div className="relative overflow-hidden">
                            ...
                            >
                                {[0, 1, 2].map((slideIndex) => (
                                    <div key={slideIndex} className="min-w-full flex justify-center md:gap-10 gap-5 flex-wrap">
                                        {books
                                            .slice(slideIndex * 3, slideIndex * 3 + 3)
                                            .map((book) => (
                                                <Card
                                                    key={book._id}
                                                    className="w-full max-w-[300px] bg-white/70 dark:bg-white/10 backdrop-blur-md shadow-xl rounded-3xl hover:scale-[1.03] hover:shadow-2xl transition-all duration-300"
                                                >
                            ...
                            <button
                                onClick={prevSlide}
                                className="absolute top-1/2 left-2 -translate-y-1/2 bg-white dark:bg-gray-700 p-3 rounded-full shadow-md hover:bg-orange-600 hover:text-white transition z-10"
                            >
                                <ChevronLeft className="w-5 h-5" />
                            </button>
                            <button
                                onClick={nextSlide}
                                className="absolute top-1/2 right-2 -translate-y-1/2 bg-white dark:bg-gray-700 p-3 rounded-full shadow-md hover:bg-orange-600 hover:text-white transition z-10"
                            >
                        ...
                        <div className="flex justify-center mt-10 space-x-3">
                            {[0, 1, 2].map((dot) => (
                                <button
                                    ...
                                    className={`h-3 w-3 rounded-full transition-all duration-300 ${currentBookSlide === dot ? 'bg-orange-600 w-5' : 'bg-gray-300'
                                        }`}
                                ></button>
                            ))}
                        </div>
